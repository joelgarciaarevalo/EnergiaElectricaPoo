...
public class GenerarFactura {
     public static void generarFactura(Cliente cliente, int mes, String archivoSalida, int diaMenorConsumo, int horaMenorConsumo, int menorConsumo, int diaMayorConsumo, int horaMayorConsumo, int MayorConsumo) {
        ...
        try {
            ...
            document.add(new Paragraph("ID del cliente: " + cliente.getId()));
            ...
            if (registradores == null || registradores.isEmpty()) {
                ...
                for (Registrador res : registradores) {
                    if (res == null || res.getConsumo() == null)
                        continue;

                    int consumo = res.getConsumo().getConsumoXMes(mes);
                    int costo = res.getConsumo().getCostoXMes(mes);
                    total += costo;

                    tabla.addCell(res.getId() != null ? res.getId() : "N/A");
